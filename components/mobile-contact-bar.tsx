...
export function MobileContactBar() {
  return (
    <div className="fixed inset-x-0 bottom-0 z-40 border-t border-fio bg-cal/95 backdrop-blur md:hidden">
      <div className="flex items-center justify-between gap-4 px-5 pt-3 pb-[max(0.75rem,env(safe-area-inset-bottom))]">
        <p className="text-sm leading-tight">
          ...
          <span className="text-tinta-suave">{site.contact.hours}</span>
        </p>
        <a
          href={whatsappUrl()}
          target="_blank"
          rel="noopener noreferrer"
          className="shrink-0 rounded-full bg-azulejo px-5 py-3 text-sm font-medium text-white"
        >
          WhatsApp
        </a>
      </div>
